...
@#for i in 1:I
    ...
    Y@{i}   - w*L@{i} - d@{i}*M@{i}                                                   = 0; // This is merely for reporting the definition Yi
    MC@{i}  - (w/(alpha@{i}*Z))*(C@{i}/(M@{i}*Z))^((1/alpha@{i})-1)                   = 0; // This is merely for reporting the definition MCi
@#endfor
