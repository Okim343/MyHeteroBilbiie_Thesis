...
Aggregate labor productivity is index by $Z_t$ (effectiveness of one unit of labor), which is 
exogenous, equal accross sectors, and follows an $AR(1)$ process (in logarithms). For a firm that chooses sector $i$, the production function becomes
...
\]
such that the optimal labor demand is then given at time $t$ in sector $i$ by $l_{i,t}^*(\omega) = \left(\frac{y}{Z_t}\right)^{\frac{1}{\alpha_i}}.$

With CES preferences, firms set relative prices as constant markups over maginal costs $$\rho_{i,t}(\omega) = \mu MC_{i,t}.$$
...
$$d_{i,t} = \left( 1-\frac{1}{\mu}\right)\frac{C_{i,t}}{M_{i,t}},$$
...
\medskip
\medskip
\noindent\emph{Firm Entry with Uncertainty}
\medskip
...
of business opportunities for technology \(i\) as 
    \[
    \psi_{i,t} =X_t^{\frac{1}{1-\phi}}\,\bar{\psi}_i,
    \]
...
normally distributed with mean zero and variance $\sigma_X$. Consequently, the probability of a sucessful startup in sector $i$ is then:
$$\Pi_{i,t} = \frac{m_{i,t}}{e_{i,t}} = e_{i,t}^{\phi-1}\,\psi_{i,t}^{\,1-\phi}.$$
...
$$v_{i,t}(\omega) = E_t \sum_{s = t+1}^{\infty} \left[ \beta (1 - \delta) \right]^{s - t} \left( \frac{C_{i,s}}{C_{i,t}} \right)^{-1} d_{i,s}(\omega),$$
and thus entry occurs until firm value is equalized with entry cost, yielding the free entry condition for sector $i$ 
$$\Pi_{i,t}v_{i,t} = f_{E,t}w_t/Z_t,$$ 
...
$$M_{i,t+1} = (1-\delta)(M_{i,t} + \Pi_{i,t}e_{i,t}).$$

An important observation is that, as the demostrated in BGM, there is no option value for waiting, since there remains no 
...
Despite the introduction of matching frictions, every potetial entrant faces the same matching probabilities $\Pi_{i,t}$ for each sector $i$ 
...
\medskip
\medskip
\noindent\emph{Preference Specification and Markups}
\medskip
...
$\mu(M_{i,t}) = \mu = \frac{\theta}{\theta - 1}, \  \rho(M_{i,t}) = M_{i,t}^{\mu - 1} = \left(M_{i,t}\right)^{\frac{1}{\theta - 1}}, 
\   \epsilon_{\text{within}}(M_{i,t}) = \mu - 1 , \ \epsilon_{\text{across}}(N_{t}) = \eta - 1$

\medskip
\medskip
\noindent\emph{Household Problem}
\medskip
...
yielding the Euler equation for bonds $$(C_t)^{-1} = \beta (1 + r_{t+1}) E_t \left[ (C_{t+1})^{-1} \right],$$ 
the Euler equation for share holdings $$v_t = \beta (1 - \delta) E_t \left[ \left( \frac{C_{t+1}}{C_t} \right)^{-1} (v_{t+1} + d_{t+1}) \right],$$ 
which if iterated forward yields the aggregate version of the firm valuation equation seen before, and finally the intratemporal 
first-order condition $$\chi\,L_{t}^{1/\varphi}=\frac{w_t}{C_t},$$ where $\varphi$ represents the frisch-elasticity of labor and $\chi$ is the
disutility of labor, as before.

\medskip
\medskip
\noindent\emph{Aggregate Accounting, Equilibrium and the Labor Market}
\medskip
...
equilibrium conditions to the household budget constraint and desaggregating firm-level variables, I arrive at the aggregate accounting identity
$$C_t + v_t\mathcal{E}_t= w_t.L_t + d_tN_t \;\Leftrightarrow\;C_t + \sum_iv_{i,t}e_{i,t}= w_t.L_t + \sum_id_{i,t}M_{i,t},$$ which states that the 
expenditure in consumption and investment in new firms must be equal to the income derived from labor and firm dividends in equilibrium.

Similar to the dynamic in the BGM benchline model, and in contrast to the standard one-sector RBC model of \textcite{campbell1994inspecting}, 
the model presented here is has two "macro-sectors", such that one employs part of the labor supply to produce consumption goods and the other
...
up new production lines, such that entry at time $t$ affects labor demand in time $t+1$. Matching frictions, however, introduce a dynamic where
not all the labor spent in building new firms is actually fruitful. Labor is used at producing firms that might not actually survive the matching
process, especially for highly scalable sectors that have lower probabilities of succesfull entry.
