...
and a role for subsidies to startup formation. However, other potential distortions such as markups, fixed costs and financial frictioins might complicate the picture, 
...
to reduce variety and future supply. \textcite{baqaee2020productivity} also discuss the possibility that frictions caused from
markup dispersions or any friction to resource reallocation can potentially cause within-sector missallocations, creating an allocative-efficiency externality.
...
To that intent, I embed the Sedláček-Sterk technology choice and matching into the Bilbiie-Ghironi-Melitz RBC with endogeneous entry framework, and explore
...
the main finding in new magnitude of the reponse of aggregate output to the temporary TFP shock. Output jumps up around 11\%  (1-2\% in BGM) due to 
...
Finally, the findings echoo the original BGM interpretation that entry behaves like capital investment in a classic RBC, such that the total number of
incumbents evolves with time-to-build and depreciation-like exit, strenghtening persistance. On top of that, however, the expanded model allows 
...
which further bolsters persistance in the model. Steady-state entry dynamics also show decreasing "returns-to-entry", where each successive sector is 
...
nudging entrants to internalise the consumer-surplus gains that new varieties generate based on their sector.
...
potential endogenous markup/price dispersion channels that naturally play an important role in firm dynamics. Third, all firms, not matter their scalability,
face the same uniform exogenous exit probability, biasiang competitive pressure and persistance. Fourth, the lack of capital shuts down capital-adjustment
dynamics that usually amplify RBC persistance. Finally, ignoring household heterogeneity and portfolio risk may understate the distributional
...
Overall, recognizing firm differences in scalability and the resulting matching frictions enchances how 
...
the mathing probability at time $t$ for sector $i$, and that firms are no characterized by both $i$ and $\omega$.
