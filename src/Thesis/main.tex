...
production activities in recession periods which takes scaling differences into consideration.
...
substantially increases when compared to the baseline RBC framework in BGM. This new magnitude comes from two new shock transmission channels: labor 
...
like BGM and \textcite{ghironi2007trade} have given substantial attention to endogeneizing entry dynamics, explaining the pro-cyclicality of net-entry in 
business cycles.
...
second moments of macro data at least as well as the standard RBC model without the introduction of capital, and even better than it was once capital is introduced. 
...
but incumbent firms now decide whether to adopt new technologies, effectively separating variety creation from the firm dynamics.
...
and enrich the classic RBC shock propagation mechanisms. The model delivers a better picture of how aggregate shocks can affect different types of firms in the economy, and
...
which implies a consumption-based price index and constant markups for the firm. Solving the firm's pricing problem arrives at the equilibrium pricing
condition of
...
I accommodate the business idea/technology choice mechanism by dividing the firm side of the economy into a finite number of sectors indexed by
...
Before to entry, the firm pays $f_{E,t}$ effective labor units, equal to $\frac{w_tf_{E,t}}{Z_t}$ units of the consumption good. 
...
profits $d_{i,s}$ in every period $s\ge t+1$ as well as $\delta$ $\forall t$. Prospective entrants choose a technology type $i$ and the corresponding sector.
...
Investment, on the one hand, now captures the replacement principle seen in the classical RBC model 
...
rather than a stand-alone legal entity, as this is the interpretation favored in the baseline model without 
...
entrants and successful ones, which is captured in Figure \ref{fig:steady-entry-log}. The horizontal axis represents the steady-state number of prospective
...
In this section, I will focus on exploring the mechanism of shock propagation in the model using a quantitative exercise.
...
to a yearly base. All calibration values and their targets can be found in Table \ref{tab:calib_params_rotated}.
...
meaning that a rise in TFP yields higher returns. Due to the time-to-build lag, $M_i$ raises subsequently, but because
...
labor used in building new product lines can be seen, especially in high-$\alpha$ sectors since entry is higher demanding more
hours.
...
increase in $Z$ leads to bigger future profits and therefore bigger net present value of entering across all sectors, as can be
seen in Figure \ref{fig:multiprod}. Due to time-to-build lag, firms pay the entry cost today but only start producing
next period. Once this new cohort starts producing, the over-entry that happens in particular in highly scalable sectors leads
